...
async function getCommonQuest() {
    ...
    if (chosenKey=="regionalElite4") {
        let selectedMon = await getBasicPokemon()
        questString = 'DEFEAT the Elite Four or regional equivalent using only pokemon from <a href="https://www.serebii.net/pokemon/gen'+selectedMon.Generation+'pokemon.shtml">gen '+ selectedMon.Generation + '</a>.'
        questType = 'battle'
    }
    ...
    // all shiny leader
    if (chosenKey=="hatchStarter") {
        let selectedMon = await getBasicPokemon()
        questString = 'HATCH a shiny OR perfect IV STARTER pokemon from <a href="https://www.serebii.net/pokemon/gen'+selectedMon.Generation+'pokemon.shtml">gen '+ selectedMon.Generation + '</a>.'
        questType = 'HATCH'
    }
